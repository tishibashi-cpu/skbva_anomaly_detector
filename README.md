...
新しいプログラム群は **トップ（`~/files3`）** に置き、末次さんのプログラム本体と実行時データは
**`legacy/`** にまとめています。

```
~/files3/                            ← ここから各プログラムを起動
...
ログインシェルは tcsh。既定の `python` は古く使えないので、`~/.cshrc` にエイリアスを足します。

```tcsh
echo "alias python /cont/python/x86_64-AlmaLinux9/3.9.17/bin/python3" >> ~/.cshrc
source ~/.cshrc
which python          # → /cont/python/.../3.9.17/bin/python3 ならOK
...
```tcsh
/cont/python/x86_64-AlmaLinux9/3.9.17/bin/python3 -m pip install h5py --user
```

> 注意：`nohup` やバックグラウンド実行ではエイリアスが効かないので、**python3 はフルパスで**呼んでください
> （エイリアス頼みだと Python 2 を拾って `SyntaxError` になります）。
...
二重起動はできません。すでに起動中に再度起動すると「既に PID xxxx で起動中」と出て終了します。
...
# アボートトリガ常駐（推奨：アボートで即解析＋定期チェック）。tcsh なので >& とフルパス
nohup /cont/python/x86_64-AlmaLinux9/3.9.17/bin/python3 detector_headless.py --watch >& watch.log &
tail -f watch.log
...
共用機の負荷を下げたい場合は優先度を落として実行できます。

```tcsh
nohup /usr/bin/nice -n 19 /cont/python/x86_64-AlmaLinux9/3.9.17/bin/python3 detector_headless.py --watch >& watch.log &
```
...
| 実行すると Python 2 で `SyntaxError`（with 文など） | nohup でエイリアスが効かず Python 2 を拾った。python3 をフルパスで呼ぶ。 |
