...
- **検知プログラム CPU / Mem**：`detector_headless.py`（`--watch`等）の使用量。ダッシュボードとは
  ...
  ときは「検知プログラム: 停止中/未検出」と表示される。CPU%・Mem%（カッコ内）はいずれも
  **サーバー全体に対する割合**なので、左の全体CPU%・Memと直接比較できる（目安:
  1プロセスでCPUが20%以上なら黄色・50%以上なら赤色、Memは15%以上なら黄色・30%以上なら赤色。
  他ユーザーへの影響の目安については第10章「他ユーザーへの影響の目安」参照）。
...
### CPU/メモリ使用量の定期ログ（`detector_resource_log.csv`）

実機で「検知プログラムのメモリがサイクルを重ねるごとに少しずつ増えていく」現象が観測されたため、
自プロセス（detector_headless.py 自身）のCPU%・RSSメモリを定期的にCSVへ記録する機能がある
（第10章のトラブルシューティング表も参照）。`--watch`・引数無しの定期ループの両方で自動的に動く
（`--once`/`--ip-judge`等の単発コマンドでは記録しない。常駐運用の長期推移を見るための機能のため）。

```bash
# 記録先: detector_headless.py と同じディレクトリの detector_resource_log.csv
tail -f detector_resource_log.csv
```

出力はヘッダ付きCSV（`timestamp,elapsed_sec,cpu_percent,rss_mb,mem_percent,nproc`）で、既定10分おきに
1行追記します。`elapsed_sec` はそのプロセスの起動からの経過秒数なので、`--watch` を再起動すると0に
戻り、再起動したタイミングも分かります。`cpu_percent`/`mem_percent`はダッシュボードの
「検知プログラムCPU/Mem」と同じくサーバー全体を100%とした割合（`top`のコア基準ではない。第3章参照）。
`mem_percent`は「他ユーザーへの影響の目安」を長期間追う目的で追加した列（第10章参照）。

- **記録間隔**: 環境変数 `RESOURCE_LOG_INTERVAL_SEC`（既定600=10分）で調整可能。
- **肥大化防止**: ファイルサイズが5MBを超えたら、直近2万行（10分間隔なら約139日分）だけ残して
  自動的に間引く。手動でのログローテーション設定は不要。
- **旧形式からの自動移行**: `mem_percent`列が無い旧形式（5列）のログファイルが既にある場合、
  起動時に自動的に新形式（6列）へ移行される（旧行の`mem_percent`は当時のシステム全体メモリ量が
  記録されていないため空欄になるが、それ以降の行からは記録される）。
- 表計算ソフトやPythonのpandas等に読み込んで、時系列でメモリの推移をグラフ化すると、
  「頭打ちになっているか」「青天井に増え続けているか」を判断しやすい。
...
| ダッシュボードの「検知プログラムMem」がサイクルを重ねるごとに少しずつ増えていく | `legacy/`のCCG本体（末次さんのオリジナル、無改変で使用）が、判定サイクルのたびに6個の`.h5`モデルファイルを`keras.models.load_model()`で毎回**再読込**する作りになっている。コード側は`keras.backend.clear_session()`を呼んで後片付けを試みているが、TensorFlowのメモリアロケータは`clear_session()`後もOS へメモリを返さずプールし続ける既知の特性があり、これはTensorFlow自体の挙動でPython側からは制御しづらい。この挙動は末次さんのオリジナルコードに起因するため`legacy/`は無改変方針上直接は修正していない。実用上は、増加が**ある水準で頭打ちになる**なら（多くの環境ではそうなる）問題ない。青天井に増え続けて共用サーバーのメモリを圧迫するようなら、`--watch`を定期的に再起動する運用（例: cronで週1回、深夜に再起動）で回避できる。ダッシュボードのその場の値だけでなく、`detector_resource_log.csv`（第II部7章）で長期の推移を確認すると「頭打ちか青天井か」を判断しやすい。 |
| アーカイバ停止中でも `--watch` がCPUを消費し続けているように見える | 実機で確認・一部修正済み。温度計異常検知（`temp_headless.py`）・流量計異常検知（`flow_headless.py`）は元々、全PVが取得0本なら`archiver_stopped`と判定してすぐ終わる（第10章の別項参照）。ただし機器劣化検知（`temp_equipment.py`）はこれとは別に、learn済みリングごとに**自前で同規模（数百〜1500本超）のフル取得を毎回試みてから**失敗を判定しており、この「試みる」こと自体がアーカイバ停止中でも無駄なkblogrd呼び出し・CPU消費になっていた。現在は、温度計異常検知が直近2時間以内に同じリングでアーカイバ停止を確認済みなら、機器劣化検知はその結果を借用して自前の取得を丸ごとスキップするようにした（詳細は第II部14章）。なお、これでカバーされるのはLER/HERのみで、IRリング（温度計異常検知の対象に無い）は12本と少数のため元々の負荷は小さい。 |

### 他ユーザーへの影響の目安

共用サーバーなので「どれくらいCPU/メモリを使うと他の人に迷惑になるか」は気になるところだと思います。
絶対的な正解は無い（サーバーの総メモリ量・同時に使っている人数・KEKの運用ルールによる）ですが、
判断材料と確認方法をまとめます。

**このダッシュボードが内部で使っている目安**（`sysload.py`）：システム全体のメモリ使用率が
**75%で警告（黄）・90%で危険（赤）**、というのが「負荷」ピルの色分けの基準です。1プロセス
（検知プログラム）単体については、もっと保守的に**メモリ15%で警告・30%で危険**、CPUは
**20%で警告・50%で危険**、を「検知プログラムCPU/Mem」ピルの色分けに使っています（複数のプロセスが
同居する共用サーバーで、1プロセスがシステム全体の1/4〜1/3を占めるのはさすがに大きすぎる、という
考え方です）。これらの数字はあくまで本プログラムが内部的に置いた目安で、KEKとしての公式な
基準ではない点に注意してください。

**実機で確認する方法**：
```bash
free -h                    # サーバー全体の総メモリ・使用量・空き（人間が読みやすい単位）
cat /proc/meminfo | head    # 同じ情報を生の数字で
swapon --show               # スワップが発生していないか（発生していれば全ユーザーへの影響が大きい）
top                          # 誰が何を使っているか一覧（他ユーザーのプロセスも見える）
```
特に**スワップが発生しているかどうか**は、単なる使用率よりも重要な警告サインです。物理メモリが
逼迫してスワップが始まると、そのサーバーを使っている**全員**の動作が急激に重くなります
（自分のプロセスが原因でなくても影響を受けます）。`vmstat 1`で`si`/`so`列が0でなければ
スワップが発生しています。

**最終的には**、`kekb-co-user01/02/03`のような共用計算機の具体的な人数・総メモリ量・
既存の運用ルール（例えば「1人1プロセスあたり何GBまで」といった申し合わせが既にあるか）は
このプログラム側からは分からないので、心配であれば真空グループやKEK計算機管理の担当者に
一度確認されるのが確実です。
...
- `dashboard.py` は `temp_detector/temp_equipment_state.json` を読み、「機器劣化検知」タブに
  ...
  **無駄なフル取得の回避**：機器劣化検知は温度計異常検知と全く同じ温度アーカイバを使う。
  実機で、アーカイバ停止中でも機器劣化検知のjudgeが毎回1000本超のフル取得を自前で試みて
  （失敗すると分かっているのに）CPU/kblogrd負荷を浪費していることが確認されたため、
  `run_periodic_judge`は各リングのjudgeを始める前に、温度計異常検知が**直近2時間以内**に
  同じリングでアーカイバ停止を確認済みなら、その結果を借用してフル取得そのものをスキップする
  （`archiver_stopped_borrowed: true`としてJSONに記録され、ダッシュボードのバナーにもその旨が
  出る）。温度計側の記録が古い/無い/停止していないと言っている場合は、これまでどおり自前で
  judgeを試みる（IRリングは温度計異常検知の対象に無いため、常に自前でjudgeする）。
  detector_headless.pyの段階分散リレー（温度計異常検知→機器劣化検知の順）では、通常この記録は
  数分以内の新鮮なものになる。
