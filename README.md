...
出力はヘッダ付きCSV（`timestamp,elapsed_sec,cpu_percent,rss_mb,nproc`）で、既定10分おきに1行追記します。
`elapsed_sec` はそのプロセスの起動からの経過秒数なので、`--watch` を再起動すると0に戻り、再起動した
タイミングも分かります。`cpu_percent`はダッシュボードの「検知プログラムCPU」と同じくサーバー全体を
100%とした割合（`top`のコア基準ではない。第3章参照）。
