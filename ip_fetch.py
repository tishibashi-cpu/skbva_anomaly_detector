...
# stderr 行から PV 名らしいトークンを拾う（VALIP:D01_IP_L01:CUR / BMLDCCT:CURRENT 等）。
...
def _run_kblogrd(pvs, ttime, log_group, kblogrd):
    """後方互換: 1グループ分を実行し stdout を返す（rc!=0 で RuntimeError）。
...
def _fetch_chunk(pvs, ttime, log_group, kblogrd, dropped_out=None, _depth=0):
    ...
    kblogrd は1本でも存在しない record があるとチャンク全体を rc=1 で失敗させる
    （過去窓では当時未設置/改名/撤去の PV が現行 CSV に混じる）。さらに、その診断には
    要求した PV 名ではなく内部 record DB の別エントリが表示されることがあり、stderr から
    犯人を特定できない。そこで:
    ...
    # (1) stderr が要求 PV 名を返している場合は、それを落として再取得
